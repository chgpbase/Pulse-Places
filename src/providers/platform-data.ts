...
export class PlatformData {
  ...
  /*
   * Check if url exists for bookmark and insert if not
   */
  insertOrUpdateBookmark(data: any) {
    return new Promise((resolve, reject) => {

      this.db.executeSql("SELECT id FROM bookmarks WHERE url = ?", [data.url]).then((result) => {
        if (result.rows.length == 0) {

          this.db.executeSql("INSERT INTO bookmarks (name, type, icon, header, url, api, locked, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);", [
            data.name,
            data.type,
            data.icon,
            data.header,
            ...
            this.db.executeSql("UPDATE bookmarks SET name = ?, type = ?, icon = ?, header = ?, url = ?, api = ?, updated = ? WHERE id = ?;", [
              data.name,
              data.type,
              data.icon,
              data.header,
    ...
    return new Promise((resolve, reject) => {
      ...
      if (scenarios.length > 0) {
        for (let scenario_index = 0; scenario_index < scenarios.length; ++scenario_index) {
          sqlQueries.push(["INSERT INTO scenarios (id, bookmark_id, app_id, site_id, scenario, timezone, scenario_if_id, scenario_then_id, scenario_day_id, scenario_time_id, time_start, time_end, date_start, date_end, frequency, delay, notification) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", [
            scenarios[scenario_index].id,
            bookmark_id,
            scenarios[scenario_index].app_id,
            scenarios[scenario_index].site_id,
            JSON.stringify(scenarios[scenario_index]),
            jsonApi.board.timezone,
        ...
        if(data.rows.length > 0) {
          for(let i = 0; i < data.rows.length; i++) {
            bookmarks.push({
              ...
              header: data.rows.item(i).header,
  ...
  getSlides() {
    return new Promise((resolve, reject) => {
      this.db.executeSql("SELECT b.* FROM bookmarks b LEFT JOIN interactions i ON b.id = i.bookmark_id GROUP BY b.id ORDER BY i.updated DESC;", []).then((data) => {
        ...
        if(data.rows.length > 0) {
          for(let i = 0; i < data.rows.length; i++) {
            let header = data.rows.item(i).header;
  ...
  getHistory() {
    return new Promise((resolve, reject) => {
      this.db.executeSql("SELECT i.*, s.scenario_if_id, s.scenario, b.icon FROM interactions i, bookmarks b LEFT JOIN scenarios s ON b.id = s.bookmark_id WHERE i.id = s.id GROUP BY i.id ORDER BY i.created DESC;", []).then((data) => {
  ...
  getSql(dropTables = false) {
    ...
    let sqlInsert = [
      ...
      'CREATE TABLE IF NOT EXISTS "bookmarks" ( "id" integer NOT NULL, "name" text NOT NULL, "type" text NOT NULL, "icon" text NOT NULL, "header" text, "url" text NOT NULL, "api" text, "label" text, "locked" integer, "meta" text, "created" integer NOT NULL, "updated" integer,  PRIMARY KEY("id"));',
      ...
      'CREATE TABLE IF NOT EXISTS "scenarios" ( "id" integer NOT NULL, "bookmark_id" integer NOT NULL, "app_id" integer, "site_id" integer, "scenario" text NOT NULL, "timezone" text NOT NULL, "scenario_if_id" integer NOT NULL, "scenario_then_id" integer NOT NULL, "scenario_day_id" integer NOT NULL, "scenario_time_id" integer NOT NULL, "time_start" text, "time_end" text, "date_start" text, "date_end" text, "frequency" integer NOT NULL, "delay" integer NOT NULL, "trigger_when_starts_active" integer NULL, "notification" text, "meta" text, PRIMARY KEY("id"), CONSTRAINT "fk_scenarios_bookmarks_bookmark_id" FOREIGN KEY ("bookmark_id") REFERENCES "bookmarks" ("id") ON DELETE CASCADE ON UPDATE CASCADE);',
